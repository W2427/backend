...
DROP EVENT IF EXISTS `archive_wbs_quantity_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_quantity`;
DROP PROCEDURE IF EXISTS `archive_wbs_quantity_of_project`;
DROP PROCEDURE IF EXISTS `archive_wbs_quantity_finished_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_quantity_started_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_quantity_total_daily`;
...
-- 统计对象：四级计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_quantity`(IN v_archive_time TIMESTAMP)
BEGIN
    ...
    REPEAT
      ...
      IF !v_cursor_finished THEN
        CALL `archive_wbs_quantity_of_project`(v_archive_time, v_project_id);
      END IF;
...
-- 为项目创建定时任务执行记录。
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_quantity_of_project`(
  ...
  BEGIN_LABEL:BEGIN
    ...
    VALUES (@schedule_id, v_project_id, 'STORED_PROCEDURE', 'ARCHIVE_WBS_QUANTITY', '数据归档：建造计划 / 建造完成量', 'RUNNING');
    ...
    WHERE `project_id` = v_project_id
      ...
      AND `archive_type`  = 'WBS_QUANTITY';

    -- 归档已完成数据
    CALL `archive_wbs_quantity_finished_daily`(v_archive_time, v_project_id);
    -- 归档已开始数据
    CALL `archive_wbs_quantity_started_daily`(v_archive_time, v_project_id);
    -- 归档总数据
    CALL `archive_wbs_quantity_total_daily`(v_archive_time, v_project_id);
...
-- 统计已完成计划
-- -----------------------------------------------------------------------------
  CREATE PROCEDURE `archive_wbs_quantity_finished_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
...
-- 统计已开始计划
-- -----------------------------------------------------------------------------
  CREATE PROCEDURE `archive_wbs_quantity_started_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
    ...
    DECLARE v_wbs_cursor  CURSOR FOR (
       ...
       FROM (
	  SELECT
		YEAR(we.`started_at`)  AS `year`,
		MONTH(we.`started_at`) AS `month`,
		DAY(we.`started_at`)   AS `day`,
		WEEK_OF_YEAR(we.`started_at`)  AS `week`,
		we.`sector`,
		we.stage,
		we.`process`,
		org.`name`,
	...
	WHERE we.`project_id` = v_project_id
	  AND we.`type` = 'ENTITY'
	  AND we.`active` = 1
	  AND (we.`running_status` = 'APPROVED' OR we.`running_status` = 'RUNNING' OR we.`running_status` = 'REJECTED')
...
-- 统计全部计划
-- -----------------------------------------------------------------------------
  CREATE PROCEDURE `archive_wbs_quantity_total_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
...
-- 执行频率：每日凌晨00:15
-- -----------------------------------------------------------------------------
CREATE EVENT `archive_wbs_quantity_daily`
  ON SCHEDULE EVERY 24 HOUR STARTS TIMESTAMP '2018-11-01 00:15:00'
  ON COMPLETION PRESERVE ENABLE
DO
  BEGIN
    -- 执行基准时间是当前时间之前6小时
    CALL `archive_wbs_quantity`(ADDTIME(NOW(), -60000));
  END $$
