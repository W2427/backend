...
DROP EVENT IF EXISTS `archive_wbs_spool_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_spool`;
DROP PROCEDURE IF EXISTS `archive_wbs_spool_of_project`;
DROP PROCEDURE IF EXISTS `archive_wbs_spool_finished_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_spool_total_daily`;
...
-- 统计对象：四级计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_spool`(IN v_archive_time TIMESTAMP)
BEGIN
    ...
    REPEAT
      ...
      IF !v_cursor_finished THEN
        CALL `archive_wbs_spool_of_project`(v_archive_time, v_project_id);
      END IF;
...
-- 为项目创建定时任务执行记录。
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_spool_of_project`(
  ...
  BEGIN_LABEL:BEGIN
    ...
    VALUES (@schedule_id, v_project_id, 'STORED_PROCEDURE', 'ARCHIVE_WBS_SPOOL', '数据归档：建造计划 / 单管施工完成量', 'RUNNING');
    ...
    WHERE `project_id` = v_project_id
      ...
      AND `archive_type`  = 'WBS_SPOOL';

    -- 归档已完成数据
    CALL `archive_wbs_spool_finished_daily`(v_archive_time, v_project_id);

    -- 归档总数据
    CALL `archive_wbs_spool_total_daily`(v_archive_time, v_project_id);
...
-- 统计已完成计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_spool_finished_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
    ...
    DECLARE v_team_name       VARCHAR(255);
  	DECLARE v_pressure_test_package_no   VARCHAR(255);
  ...
  `e`.`process`,
  `e`.`name`,
  COUNT(e.id) AS countt,
  e.pressure_test_package_no
FROM
  (SELECT
  ...
  WHERE we.`project_id` = v_project_id
    ...
    WHERE eptp.id = pn.entity_id
      AND hn.node_id = pn.id
      AND hn.id = hniso.parent_id
      AND hniso.node_id = pniso.id)) AS e
GROUP BY `e`.`year`,
  `e`.`month`,
  `e`.`day`,
  `e`.`week`,
       ...
       v_pressure_test_package_no
        ...
        IF !v_cursor_finished AND v_group_year IS NOT NULL THEN
          INSERT INTO `statistics` (
            ...
            UPPER(MD5(CONCAT(
              v_project_id, '\WBS_SPOOL\nDAILY\n',
              CONCAT(@archive_year, '\n', @archive_month, '\n', @archive_day, '\n'),
              CONCAT(v_group_year, '\n', v_group_month, '\n', v_group_day, '\n'),
              CONCAT(v_sector, '\n', v_stage,'\n', v_process),
              IFNULL(v_team_name, ''),
			  IFNULL(v_pressure_test_package_no, '')
            ))),
            v_project_id,
            'WBS_SPOOL',
            'DAILY',
...
-- 统计全部计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_spool_total_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
...
-- 执行频率：每日凌晨01:00
-- -----------------------------------------------------------------------------
CREATE EVENT `archive_wbs_spool_daily`
  ON SCHEDULE EVERY 24 HOUR STARTS TIMESTAMP '2018-11-01 01:00:00'
  ON COMPLETION PRESERVE ENABLE
DO
  BEGIN
    -- 执行基准时间是当前时间之前6小时
    CALL `archive_wbs_spool`(ADDTIME(NOW(), -60000));
  END $$
