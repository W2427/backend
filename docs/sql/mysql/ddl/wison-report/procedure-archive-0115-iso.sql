...
DROP EVENT IF EXISTS `archive_wbs_iso_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_iso`;
DROP PROCEDURE IF EXISTS `archive_wbs_iso_of_project`;
DROP PROCEDURE IF EXISTS `archive_wbs_iso_finished_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_iso_total_daily`;
...
-- 统计对象：四级计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_iso`(IN v_archive_time TIMESTAMP)
BEGIN
    ...
    REPEAT
      ...
      IF !v_cursor_finished THEN
        CALL `archive_wbs_iso_of_project`(v_archive_time, v_project_id);
      END IF;
...
-- 为项目创建定时任务执行记录。
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_iso_of_project`(
  ...
  BEGIN_LABEL:BEGIN
    ...
    VALUES (@schedule_id, v_project_id, 'STORED_PROCEDURE', 'ARCHIVE_WBS_ISO', '数据归档：建造计划 / 管线施工完成量', 'RUNNING');
    ...
    WHERE `project_id` = v_project_id
      ...
      AND `archive_type`  = 'WBS_ISO';

    -- 归档已完成数据
    CALL `archive_wbs_iso_finished_daily`(v_archive_time, v_project_id);

    -- 归档总数据
    CALL `archive_wbs_iso_total_daily`(v_archive_time, v_project_id);
...
-- 统计已完成计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_iso_finished_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
...
FROM
  ...
  FROM
    ...
    INNER JOIN ose_tasks.entity_iso i
      ON we.`entity_id` = i.`id`
      AND i.`status` = 'ACTIVE'
      AND i.`deleted` = 0
    LEFT JOIN
      (SELECT
...
-- 统计全部计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_iso_total_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
...
-- 执行频率：每日凌晨01:15
-- -----------------------------------------------------------------------------
CREATE EVENT `archive_wbs_iso_daily`
  ON SCHEDULE EVERY 24 HOUR STARTS TIMESTAMP '2018-11-01 01:15:00'
  ON COMPLETION PRESERVE ENABLE
DO
  BEGIN
    -- 执行基准时间是当前时间之前6小时
    CALL `archive_wbs_iso`(ADDTIME(NOW(), -60000));
  END $$
