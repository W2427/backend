...
DROP EVENT IF EXISTS `archive_wbs_clean_package_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_clean_package`;
DROP PROCEDURE IF EXISTS `archive_wbs_clean_package_of_project`;
DROP PROCEDURE IF EXISTS `archive_wbs_clean_package_finished_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_clean_package_total_daily`;
...
-- 统计对象：四级计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_clean_package`(IN v_archive_time TIMESTAMP)
BEGIN
    ...
    REPEAT
      ...
      IF !v_cursor_finished THEN
        CALL `archive_wbs_clean_package_of_project`(v_archive_time, v_project_id);
      END IF;
...
-- 为项目创建定时任务执行记录。
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_clean_package_of_project`(
  ...
  BEGIN_LABEL:BEGIN
    ...
    WHERE
      `code` = 'ARCHIVE_WBS_CLEAN_PACKAGE'
    ...
    VALUES (@schedule_id, v_project_id, 'STORED_PROCEDURE', 'ARCHIVE_WBS_CLEAN_PACKAGE', '数据归档：建造计划 / 清洁进度', 'RUNNING');
    ...
    WHERE `project_id` = v_project_id
      ...
      AND `archive_type`  = 'WBS_CLEAN_PACKAGE';

    -- 归档已完成数据
    CALL `archive_wbs_clean_package_finished_daily`(v_archive_time, v_project_id);

    -- 归档总数据
    CALL `archive_wbs_clean_package_total_daily`(v_archive_time, v_project_id);
...
-- 统计已完成计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_clean_package_finished_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
	...
	FROM
	  ...
	  FROM
		...
		INNER JOIN ose_tasks.entity_clean_package cp
		  ON we.`entity_id` = cp.`id`
		  AND cp.`status` = 'ACTIVE'
		  AND cp.`deleted` = 0
		LEFT JOIN ose_tasks.`bpm_activity_instance` act
		  ON we.`project_id` = act.`project_id`
		  AND we.`entity_id` = act.`entity_id`
		  AND we.`process` = act.`process_name`
		  AND we.`stage` = act.`process_stage_name_cn`
		  AND act.`suspension_state` = 'ACTIVE'
		  AND act.`finish_state` = 'SUSPEND'
		  AND act.`version` = 0
	   ...
	   v_count
        ...
        IF !v_cursor_finished AND v_group_year IS NOT NULL THEN
          INSERT INTO `statistics` (
            ...
            UPPER(MD5(CONCAT(
              v_project_id, '\WBS_CLEAN_PACKAGE\nDAILY\n',
              CONCAT(@archive_year, '\n', @archive_month, '\n', @archive_day, '\n'),
              CONCAT(v_group_year, '\n', v_group_month, '\n', v_group_day, '\n'),
              CONCAT(v_sector, '\n', v_stage,'\n', v_process),
              IFNULL(v_team_name, '')
...
-- 统计全部计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_clean_package_total_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
...
-- 执行频率：每日凌晨01:45
-- -----------------------------------------------------------------------------
CREATE EVENT `archive_wbs_clean_package_daily`
  ON SCHEDULE EVERY 24 HOUR STARTS TIMESTAMP '2018-11-01 01:45:00'
  ON COMPLETION PRESERVE ENABLE
DO
  BEGIN
    -- 执行基准时间是当前时间之前6小时
    CALL `archive_wbs_clean_package`(ADDTIME(NOW(), -60000));
  END $$
