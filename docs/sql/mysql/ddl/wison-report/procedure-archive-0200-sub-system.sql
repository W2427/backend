...
DROP EVENT IF EXISTS `archive_wbs_sub_system_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_sub_system`;
DROP PROCEDURE IF EXISTS `archive_wbs_sub_system_of_project`;
DROP PROCEDURE IF EXISTS `archive_wbs_sub_system_finished_daily`;
DROP PROCEDURE IF EXISTS `archive_wbs_sub_system_total_daily`;
...
-- 统计对象：四级计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_sub_system`(IN v_archive_time TIMESTAMP)
BEGIN
    ...
    REPEAT
      ...
      IF !v_cursor_finished THEN
        CALL `archive_wbs_sub_system_of_project`(v_archive_time, v_project_id);
      END IF;
...
-- 为项目创建定时任务执行记录。
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_sub_system_of_project`(
  ...
  BEGIN_LABEL:BEGIN
    ...
    VALUES (@schedule_id, v_project_id, 'STORED_PROCEDURE', 'ARCHIVE_WBS_SUB_SYSTEM', '数据归档：建造计划 / 系统密性进度', 'RUNNING');
    ...
    WHERE `project_id` = v_project_id
      ...
      AND `archive_type`  = 'WBS_SUB_SYSTEM';

    -- 归档已完成数据
    CALL `archive_wbs_sub_system_finished_daily`(v_archive_time, v_project_id);

    -- 归档总数据
    CALL `archive_wbs_sub_system_total_daily`(v_archive_time, v_project_id);
...
-- 统计已完成计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_sub_system_finished_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
	...
	FROM
	  ...
	  FROM
		...
		INNER JOIN ose_tasks.entity_sub_system ss
		  ON we.`entity_id` = ss.`id`
		  AND ss.`status` = 'ACTIVE'
		  AND ss.`deleted` = 0
		LEFT JOIN ose_tasks.`bpm_activity_instance_base` act
    ...
    LEFT JOIN ose_tasks.bpm_activity_instance_state acts
      ON act.project_id = acts.project_id
      AND act.id = acts.bai_id
		LEFT JOIN ose_auth.organizations org
		  ON org.id = act.team_id
	  WHERE we.`project_id` = v_project_id
	  AND acts.`suspension_state` = 'ACTIVE'
		AND acts.`finish_state` = 'FINISHED'
		AND we.`type` = 'ENTITY'
		AND we.`active` = 1
...
-- 统计全部计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_wbs_sub_system_total_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
...
-- 执行频率：每日凌晨02:00
-- -----------------------------------------------------------------------------
CREATE EVENT `archive_wbs_sub_system_daily`
  ON SCHEDULE EVERY 24 HOUR STARTS TIMESTAMP '2018-11-01 02:00:00'
  ON COMPLETION PRESERVE ENABLE
DO
  BEGIN
    -- 执行基准时间是当前时间之前6小时
    CALL `archive_wbs_sub_system`(ADDTIME(NOW(), -60000));
  END $$
