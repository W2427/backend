...
DROP EVENT IF EXISTS `archive_wbs_design_rate_daily`;
DROP PROCEDURE IF EXISTS `archive_design_rate`;
DROP PROCEDURE IF EXISTS `archive_design_rate_of_project`;
DROP PROCEDURE IF EXISTS `archive_design_rate_finished_daily`;
DROP PROCEDURE IF EXISTS `archive_design_rate_plan_daily`;
...
-- 统计对象：四级计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_design_rate`(IN v_archive_time TIMESTAMP)
BEGIN
    ...
    REPEAT
      ...
      IF !v_cursor_finished THEN
        CALL `archive_design_rate_of_project`(v_archive_time, v_project_id);
      END IF;
...
-- 为项目创建定时任务执行记录。
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_design_rate_of_project`(
  ...
  BEGIN_LABEL:BEGIN
    ...
    VALUES (@schedule_id, v_project_id, 'STORED_PROCEDURE', 'ARCHIVE_DESIGN_RATE', '数据归档：生产设计 / 完成率', 'RUNNING');
    ...
    WHERE `project_id` = v_project_id
      ...
      AND `archive_type`  = 'DESIGN_RATE';

    -- 归档已完成数据
    CALL `archive_design_rate_finished_daily`(v_archive_time, v_project_id);

    -- 归档计划数据
    CALL `archive_design_rate_plan_daily`(v_archive_time, v_project_id);
...
-- 统计已完成
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_design_rate_finished_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
...
-- 统计计划
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_design_rate_plan_daily`(
  IN v_archive_time TIMESTAMP,  -- 归档时间
  IN v_project_id   BIGINT(20) -- 项目 ID
)
BEGIN
		...
		FROM
		  ...
		  FROM
			...
			LEFT JOIN ose_tasks.`bpm_entity_sub_type` c
			  ON p.`drawing_category_id` = c.`id`
		   WHERE p.`project_id` = v_project_id
			AND p.`engineering_finish_date` IS NOT NULL
			AND p.`status` = 'ACTIVE') e
		GROUP BY `e`.`year`,
		  `e`.`month`,
		  `e`.`day`,
...
-- 执行频率：每日凌晨03:00
-- -----------------------------------------------------------------------------
CREATE EVENT `archive_wbs_design_rate_daily`
  ON SCHEDULE EVERY 24 HOUR STARTS TIMESTAMP '2018-11-01 03:00:00'
  ON COMPLETION PRESERVE ENABLE
DO
  BEGIN
    -- 执行基准时间是当前时间之前6小时
    CALL `archive_design_rate`(ADDTIME(NOW(), -60000));
  END $$
