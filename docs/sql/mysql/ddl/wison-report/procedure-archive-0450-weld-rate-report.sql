...
DROP EVENT IF EXISTS `archive_weld_rate_daily`;
DROP PROCEDURE IF EXISTS `archive_weld_rate`;
DROP PROCEDURE IF EXISTS `archive_weld_rate_of_project`;
...
-- 统计对象：完成焊接的焊口
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_weld_rate`(IN v_archive_time TIMESTAMP)
  BEGIN_LABEL:BEGIN
    ...
    REPEAT
      ...
      IF !v_cursor_finished THEN
        -- 按项目归档数据
        CALL `archive_weld_rate_of_project`(v_archive_time, v_project_id);
      END IF;
...
-- -----------------------------------------------------------------------------
CREATE PROCEDURE `archive_weld_rate_of_project`(
  ...
  BEGIN_LABEL:BEGIN
    ...
    DECLARE v_weld_type             VARCHAR(255); -- 焊接类型
    DECLARE v_welder_no             VARCHAR(255); -- 焊工号
    DECLARE v_qualified_nps         DECIMAL(19,3); -- 焊接合格寸径
    DECLARE v_failed_nps            DECIMAL(19,3); -- 焊接不合格寸径
    DECLARE v_qualified_count       INTEGER; -- 焊接合格数量
    DECLARE v_failed_count          INTEGER; -- 焊接不合格数量

    DECLARE v_weld_rate_cursor  CURSOR FOR (
		SELECT
		  tt.`module`,
		  tt.`stage`,
		  tt.`entity_material`,
		...
		FROM
		  ...
		  FROM
			...
			JOIN `ose_tasks`.`wbs_entry` `wbs`
			  ...
			  AND `wbs`.`process` = 'WELD'
			JOIN `ose_auth`.`organizations` AS `org`
			  ON `wbs`.`team_id` = `org`.`id`
			JOIN `ose_tasks`.`welder` AS `welder`
			  ON `w`.`project_id` = `welder`.`project_id`
			  AND `w`.`welder_id` = `welder`.`id`
			JOIN `ose_tasks`.`bpm_activity_instance` AS `act`
			  ON `w`.`project_id` = `act`.`project_id`
			  AND `w`.`id` = `act`.`entity_id`
			  AND `act`.`a.process_name` = 'WELD'
			  AND `act`.`external_inspection_fpy` <> 0
		  ...
		  WHERE `w`.`project_id` = v_project_id
			...
			(SELECT
			  `id`
			FROM
			  `ose_tasks`.`bpm_activity_instance` `act2`
			WHERE `act2`.`project_id` = `w`.`project_id`
			  AND `act2`.`entity_id` = `w`.`id`
			  AND `act2`.`a.process_name` = 'WELD'
			  AND `act2`.`external_inspection_fpy` <> 0
			  AND `act2`.`end_date` IS NOT NULL)) AS tt
		GROUP BY tt.`module`,
		  tt.`stage`,
		  tt.`entity_material`,
		  tt.`team_name`,
    ...
    WHERE
      `code` = 'ARCHIVE_WBS_WELD_RATE_PROGRESS'
    ...
    VALUES (@schedule_id, v_project_id, 'STORED_PROCEDURE', 'ARCHIVE_WBS_WELD_RATE_PROGRESS', '数据归档：质量控制 / 焊接合格率', 'RUNNING');
    ...
    WHERE `project_id` = v_project_id
        ...
        AND `archive_type`  = 'WBS_WELD_RATE_PROGRESS';

    OPEN v_weld_rate_cursor;
    REPEAT
...
-- 执行频率：每日凌晨4点50分
-- -----------------------------------------------------------------------------
CREATE EVENT `archive_weld_rate_daily`
  ON SCHEDULE EVERY 24 HOUR STARTS TIMESTAMP '2018-11-01 04:50:00'
  ON COMPLETION PRESERVE ENABLE
DO
  BEGIN
    -- 执行基准时间是当前时间之前6小时
    CALL `archive_weld_rate`(ADDTIME(NOW(), -60000));
  END $$
