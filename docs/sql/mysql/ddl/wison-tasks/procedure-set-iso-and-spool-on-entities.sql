...
-- 为实体设置上级实体信息。
--
-- 根据实体层级信息取得实体的上级管线实体及单管实体（如果存在），
-- 并分别将上级实体的 ID 设置到 iso_entity_id 和 spool_entity_id 字段上。
-- 管线的 iso_entity_id 为自身，单管的 spool_entity_id 为自身。
-- 同时更新项目节点（project_node）和计划条目（wbs_entry）的相应字段。
--
-- 数据库类型：MySQL
...
CREATE PROCEDURE `set_parent_entity_id_on_entities`(IN v_project_id BIGINT(20))
  BEGIN
...
CREATE PROCEDURE `set_parent_entity_id_on_wbs_entries`(IN v_project_id BIGINT(20))
  BEGIN
