...
-- ----------------------------
-- Procedure structure for shift_weld
-- ----------------------------
DROP PROCEDURE IF EXISTS `shift_weld`;
delimiter ;;
CREATE DEFINER=`oseadmin`@`%` PROCEDURE `shift_weld`(
																												IN userName varchar(50),
																												IN spOldName varchar(50),
																												IN spNewName varchar(50),
																												IN weldName varchar(50),
...
-- 	declare oldWelderId varchar(50);
	...
	declare weldId BIGINT(20);
	declare spNewId BIGINT(20);
	declare weldNewName varchar(50);
	declare weldNo varchar(50);
	declare weldHierarchyParentId BIGINT(20);
...
-- 		select 'fyx-ftj';
	...
	-- 根据weld的no，取得weld_id
	select id,display_name into weldId,weldNo from entity_weld where no=@weldName and project_id = projectId and deleted=0;
-- 		select ppName,ppId,ppNo;
		if isnull(weldId) then
		select 'step2';
		leave leave_proc;
		end if;
...
-- 		select 'fftj';
	...
	-- 建立新的weld的名字为 SP_NEW+P+000NO。
-- 	select concat(spNewName,substr('-P0000',1,6-length(weldNo)),weldNo) into weldNewName;
--  	select weldNewName;
...
-- 	select ppHierarchyParentId;
	...
	-- weld的project_nodes的id
		select id into pnId from project_node where entity_id = weldId;
	...
	-- 更新weld在entity_pipe_pieces中的no，为SP_new+P+000No。
	update entity_weld set last_modified_at = now(),revision = @newRev where id = weldId;

	-- 更新 wbs_entries中的parent_hierarchy_node_id,spool_no,spool_entity_id
	update wbs_entry set parent_hierarchy_node_id = spNewLpHnId,spool_no=spNewName,spool_entity_id=spNewId where project_id = projectId and entity_id = weldId;

	-- 更新bpm_activity_instances的 entity_no为新名称。
-- 	update bpm_activity_instance set entity_no = weldNewName where entity_id = weldId;

	-- 修改project_node的node_no
	update project_node set spool_no=spNewName,spool_entity_id=spNewId where id = pnId;

	-- 更新entity_qr_code的entity_no
-- 	update entity_qr_code set entity_no = weldNewName where entity_id = weldId;
