...
-- ------------------------------
-- cable_tray_hierarchy_info
-- cable_tray层级视图
-- ------------------------------
CREATE OR REPLACE VIEW `cable_tray_hierarchy_info` AS
  SELECT
  ...
  FROM
    `entity_cable_tray` `cable_tray`
    JOIN `project_node` `pn`
      ON `pn`.`project_id` = `cable_tray`.`project_id`
      AND `pn`.`entity_id` = `cable_tray`.`id`
      AND `pn`.`entity_type` = 'CABLE_TRAY'
      AND `pn`.`deleted` = `cable_tray`.`deleted`
