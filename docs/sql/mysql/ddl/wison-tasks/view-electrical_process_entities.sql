...
-- ----------------------------
-- View structure for electrical_process_entities
-- ELECTRICAL 专业的 工序和实体关系视图
-- ----------------------------
CREATE OR REPLACE VIEW `electrical_process_entities` AS
  SELECT
  ...
  FROM
    ...
    `bpm_process_stage` `s`
        ...
        JOIN `project_node` `pn` ON
          ...
          AND  `pn`.`deleted` = 0
--           AND `pn`.`discipline` = 'ELECTRICAL'

        -- 取得 层级节点 根据 项目节点 （ 在 STRUCTURE维度 的层级节点）
        JOIN `hierarchy_node` `hn` ON
          `hn`.`node_id` = `pn`.`id`
          AND `hn`.`hierarchy_type` IN ('ELECTRICAL','CABLE_PULLING_PACKAGE','SUB_SYSTEM')
          AND  `hn`.`deleted` = 0
        ...
        LEFT JOIN `project_node` `ppn` ON
          ...
          AND `ppn`.`node_type`  IN ('MODULE','SYSTEM')
          AND  `ppn`.`deleted` = 0
