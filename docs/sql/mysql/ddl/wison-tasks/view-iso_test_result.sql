...
-- ----------------------------
-- View structure for iso_test_result
-- 视图 ISO测试结果
-- ----------------------------
CREATE OR REPLACE VIEW `iso_test_result` AS
  SELECT
    `ei`.`id`                       AS `id`,
    `ei`.`org_id`                   AS `org_id`,
  ...
  FROM
    ...
    `entity_iso` `ei`
    LEFT JOIN `iso_process_log` `lc`
      ON `lc`.`latest_test` = TRUE
      AND `ei`.`line_check_id` = `lc`.`id`

    LEFT JOIN `iso_process_log` `bf`
      ON `bf`.`latest_test` = TRUE
      AND `ei`.`blind_fabricate_id` = `bf`.`id`

    LEFT JOIN `iso_process_log` `bi`
      ON `bi`.`latest_test` = TRUE
      AND `ei`.`blind_install_id` = `bi`.`id`

    LEFT JOIN `iso_process_log` `pt`
      ON `pt`.`latest_test` = TRUE
      AND `ei`.`pressure_test_id` = `pt`.`id`

    LEFT JOIN `iso_process_log` `br`
      ON `br`.`latest_test` = TRUE
      AND `ei`.`blind_remove_id` = `br`.`id`

    LEFT JOIN `iso_process_log` `blow`
      ON `blow`.`latest_test` = TRUE
      AND `ei`.`blow_id` = `blow`.`id`

    LEFT JOIN `iso_process_log` `oil`
      ON `oil`.`latest_test` = TRUE
      AND `ei`.`oil_flush_id` = `oil`.`id`

    LEFT JOIN `iso_process_log` `wf`
      ON `wf`.`latest_test` = TRUE
      AND `ei`.`water_flush_id` = `wf`.`id`

    LEFT JOIN `iso_process_log` `sf`
      ON `sf`.`latest_test` = TRUE
      AND `ei`.`shield_fabricate_id` = `sf`.`id`

    LEFT JOIN `iso_process_log` `fi`
      ON `fi`.`latest_test` = TRUE
      AND `ei`.`filling_install_id` = `fi`.`id`

    LEFT JOIN `iso_process_log` `si`
      ON `si`.`latest_test` = TRUE
