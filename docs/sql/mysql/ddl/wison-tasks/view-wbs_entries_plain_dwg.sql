...
-- 	`we`.`started_at`               AS `started_at`,
	...
	`dsp`.`sub_drawing_no`          AS `sub_drawing_no`,
	`dsp`.`sub_drawing_version`     AS `sub_drawing_versiON`,
	`dsp`.`mark_deleted`            AS `mark_deleted`,
	`dsp`.`page_count`              AS `page_count`,
	`dsp`.`page_no`                 AS `page_no`,
	`dsp`.`id`                      AS `sub_drawing_id`
FROM
	`wbs_entry` `we`
		LEFT JOIN `entity_iso` `ei` ON `we`.`iso_entity_id` = `ei`.`id`

		LEFT JOIN `sub_drawing` `dsp` ON REPLACE(REPLACE ( `we`.`iso_no`, '"', '_' ), '/','_') = `dsp`.`sub_drawing_no`
