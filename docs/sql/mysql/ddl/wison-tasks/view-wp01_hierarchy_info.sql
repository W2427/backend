...
CREATE OR REPLACE VIEW `wp01_hierarchy_info` AS
SELECT
...
FROM
	`entity_wp01` `w1`
	LEFT JOIN `project_node` `pn` ON `w1`.`project_id` = `pn`.`project_id`
	AND `w1`.`id` = `pn`.`entity_id`
	AND `pn`.`deleted` = 0
	AND `pn`.`entity_type` = 'WP01'
	LEFT JOIN `hierarchy_node` `hn_on_structure` ON `hn_on_structure`.`project_id` = `pn`.`project_id`
