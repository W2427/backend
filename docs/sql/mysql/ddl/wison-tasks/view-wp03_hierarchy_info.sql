...
CREATE OR REPLACE VIEW `wp03_hierarchy_info` AS
SELECT
...
FROM
	`entity_wp03` `wp3`
	LEFT JOIN `project_node` `pn` ON `pn`.`entity_id` = `wp3`.`id`
	...
	LEFT JOIN `hierarchy_node` `parent_hn_on_structure` ON `parent_hn_on_structure`.`project_id` = `hn_on_structure`.`project_id`
	...
	LEFT JOIN `project_node` `parent_pn` ON `parent_hn_on_structure`.`project_id` = `parent_pn`.`project_id`
