...
CREATE OR REPLACE VIEW `pipe_weld_log` AS
SELECT
		...
		MAX(CASE process WHEN 'FITUP' THEN pcl.last_modified_at END) `Fit-up Date`,
		...
		MAX(CASE process WHEN 'WELD' THEN pcl.last_modified_at END) `Welding date`,
		...
		MAX(CASE process WHEN 'PWHT' THEN pcl.last_modified_at END) `Date of PWHT`,
		MAX(CASE process WHEN 'PWHT' THEN report_nos END) `PWHT Report`,
		MAX(CASE process WHEN 'WELD' THEN pcl.last_modified_at END) `Date of visual testing`,
		...
		MAX(CASE process WHEN 'MT' THEN pcl.last_modified_at END) `Date of MT testing`,
		...
		MAX(CASE process WHEN 'PT' THEN pcl.last_modified_at END) `Date of PT testing`,
		...
		MAX(CASE process WHEN 'UT' THEN pcl.last_modified_at END) `Date of UT testing`,
		...
		MAX(CASE process WHEN 'RT' THEN pcl.last_modified_at END) `Date of RT testing`,
		...
		MAX(CASE process WHEN 'PMI' THEN pcl.last_modified_at END) `Date of PMI testing`,
...
FROM piping_construction_log pcl
		LEFT JOIN mat_heat_no mhn1 ON pcl.heat_no_id1 = mhn1.id
		LEFT JOIN mat_heat_no mhn2 ON pcl.heat_no_id2 = mhn2.id
