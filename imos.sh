...
for service_name in ${all_service_names[@]}; do
	...
	if [ "${service_name}" == "ose-tasks" ]; then
		java ${profile_name} -Ddebug -Xms1024m -Xmx1024m -jar ${service_jar_file} > ${service_log_file} &
	elif [ "${service_name}" == "ose-report" ]; then
		java ${profile_name} -Ddebug -Xms512m -Xmx512m -jar ${service_jar_file} > ${service_log_file} &
	else
		java ${profile_name} -Ddebug -Xms256m -Xmx256m -jar ${service_jar_file} > ${service_log_file} &
	fi
