...
import com.ose.report.api.ExternalInspectionItemListAPI;
...
@Tag(name = "生成外检报验清单")
@RestController
public class ExternalInspectionItemListController extends BaseReportController implements ExternalInspectionItemListAPI {

    /**
     * 构造方法。
     */
    @Autowired
    public ExternalInspectionItemListController(
    ...
    @Operation(description = "生成外检报验清单")
    @RequestMapping(
        method = POST,
        value = "/orgs/{orgId}/projects/{projectId}/external-inspection-item-list"
    )
    @WithPrivilege
    public JsonObjectResponseBody<ReportHistory> generateExternalInspectionItemList(
        ...
        return new JsonObjectResponseBody<>(
            generateReportFile(
                ...
                orgId,
                projectId,
                "external-inspection-item-list.jasper",
                reportDTO
            )
        );
    }
