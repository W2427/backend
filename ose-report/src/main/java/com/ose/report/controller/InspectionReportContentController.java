...
@Tag(name = "生成检验报告目录")
@RestController
public class InspectionReportContentController extends BaseReportController implements InspectionReportContentAPI {

    /**
     * 构造方法。
     */
    @Autowired
    public InspectionReportContentController(
    ...
    @Operation(description = "生成检验报告目录")
    @RequestMapping(
        method = POST,
        value = "/orgs/{orgId}/projects/{projectId}/inspection-report-contents"
    )
    @WithPrivilege
    public JsonObjectResponseBody<ReportHistory> generateInspectionReportContent(
        ...
        return new JsonObjectResponseBody<>(
            generateReportFile(
                ...
                orgId,
                projectId,
                "inspection-report-content.jasper",
                reportDTO
            )
        );
    }
