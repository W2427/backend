 ...
 *
 */
public class SuggestionRepositoryImpl extends BaseRepository implements SuggestionRepositoryCustom {
    ...
    public Page<Suggestion> getList(SuggestionSearchDTO dto, Long operatorId) {
        ...
        sql.append("  s.`last_modified_at` AS lastModifiedAt,");
        sql.append("  s.`status`,");
        sql.append("  s.`file_id` AS fileId,");
        sql.append("  s.`file_path` AS filePath,");
        sql.append("  s.`proposer`,");
        sql.append("  s.`proposer_name` AS proposerName,");
        sql.append("  s.`file_name` AS fileName, ");
        sql.append("  s.`summary` as summary, ");
        sql.append("  s.`description` as description, ");
        sql.append("  s.`priority` as priority ");
        sql.append("FROM");
        sql.append("  `suggestion` s ");
        sql.append("WHERE s.deleted is false ");

        if (dto.getSummary() != null && !"".equals(dto.getSummary())) {
            sql.append("AND s.summary like :summary ");
        }

        if (dto.getProposer() != null && !"".equals(dto.getProposer())) {
            sql.append("AND s.proposer_name like :proposer ");
        }

        if (dto.getStatus() != null && !"All".equals(dto.getStatus())) {
        ...
        if (dto.getSummary() != null && !"".equals(dto.getSummary())) {
            query.setParameter("summary", "%" + dto.getSummary() + "%");
            countQuery.setParameter("summary", "%" + dto.getSummary() + "%");
        }

        if (dto.getProposer() != null && !"All".equals(dto.getProposer())) {
            query.setParameter("proposer", "%" + dto.getProposer() + "%");
            countQuery.setParameter("proposer", "%" + dto.getProposer() + "%");
        }
        ...
        for (Map<String, Object> wpsMap : queryResultList) {
            ...
            if (wpsMap.get("status") != null) {
                suggestion.setStatus(EntityStatus.valueOf((String) wpsMap.get("status")));
            }
            BeanUtils.copyProperties(wpsMap, suggestion,"status");
            resultList.add(suggestion);
        }

        return new PageImpl<>(resultList, dto.toPageable(), count.longValue());
    }
