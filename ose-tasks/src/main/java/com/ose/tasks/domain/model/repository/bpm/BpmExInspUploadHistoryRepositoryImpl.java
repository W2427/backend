 ...
 * 根据二维码查询得到外检申请的聚合数据。
 */
public class BpmExInspUploadHistoryRepositoryImpl extends BaseRepository implements BpmExInspUploadHistoryRepositoryCustom {
    ...
    public Page<BpmExInspUploadHistory> externalInspectionUploadHistories(Long orgId, Long projectId,
        ...
        sql.append("  his.`org_id` AS orgId,");
        sql.append("  his.`project_id` AS projectId,");
        sql.append("  his.`file_name` AS fileName, ");
        sql.append("  his.`confirmed`, ");
        sql.append("  his.`memo` ");
        sql.append("FROM");
        sql.append("  `bpm_external_inspection_upload_history` his ");
        sql.append("  LEFT JOIN `bpm_external_inspection_confirm` co ");
        sql.append("    ON his.`id` = co.upload_history_id ");
        sql.append("WHERE 1 = 1 ");
        sql.append("AND his.org_id = :orgId ");
        sql.append("AND his.project_id = :projectId ");
        sql.append("AND his.operator = :operatorId ");

        if (pageDTO.getKeyword() != null
            && !"".equals(pageDTO.getKeyword())) {
            sql.append("AND (co.report_no like :keyword OR co.series_no like :keyword ) ");
        }

        if (pageDTO.getConfirmed() != null) {
            sql.append("AND his.`confirmed` = :confirmed ");
        }
        ...
        if (pageDTO.getConfirmed() != null) {
            query.setParameter("confirmed", pageDTO.getConfirmed());
            countQuery.setParameter("confirmed", pageDTO.getConfirmed());
        }
        ...
        List<BpmExInspUploadHistory> resultList = new ArrayList<>();

        for (Map<String, Object> wpsMap : queryResultList) {
            BpmExInspUploadHistory his = new BpmExInspUploadHistory();
