...
@Component
public class ExternalDrawingImportService extends StringRedisService implements ExternalDrawingImportInterface {
    ...
    private ExternalDrawingRepository drawingRepository;
     ...
     */
    @Autowired
    public ExternalDrawingImportService(
        ExternalDrawingRepository drawingRepository,
...
//        workbook.getSheet()
        ...
        switch (discipline.toUpperCase()) {
            ...
            case "STRUCTURE":
                ...
                columnConfig.setDwgNoColumn(3);
                columnConfig.setDwgTitleColumn(4);
                columnConfig.setLatestRevColumn(5);
                columnConfig.setRatedHoursColumn(6);
                ...
                columnConfig.setDrawingCategory(12);
                columnConfig.setWorkNet(13);
                columnConfig.setSection(14);
                columnConfig.setBlock(15);
                columnConfig.setSmallArea(16);
                columnConfig.setDesignDiscipline(17);
                columnConfig.setInstallationDrawingNo(18);
                break;
    ...
    private BatchResultDTO importDrawing(Long orgId,
        ...
        Integer seqNoColumn = columnConfig.getSeqNoColumn(); //0
        ...
        while (rows.hasNext()) {
            ...
            try {
                ...
                totalCount++;
                //导入主表记录
                ExternalDrawing drawing = new ExternalDrawing();
                drawing.setBatchTaskId(batchResult.getTaskId());

                drawing.setSequenceNo(WorkbookUtils.readAsString(row, seqNoColumn));
...
//                }
                ...
                drawing.setEntitySubType(bpmEntitySubType.getNameEn());
                ...
                2.1 检查 图纸是否已经存在
                 */
                Optional<ExternalDrawing> op = drawingRepository.findByOrgIdAndProjectIdAndDwgNoAndStatus(orgId, projectId, drawing.getDwgNo(), EntityStatus.ACTIVE);
                if (op.isPresent()) {
                    op.get().setAreaName(drawing.getAreaName());
                    op.get().setModuleName(drawing.getModuleName());
                    drawingRepository.save(op.get());
                } else {
                    //如果不存在，设置属性，并保存
...
//                    drawing.setLatestRev("0");
    ...
    public boolean updateFileId(Long batchTaskId, Long fileId) {
        List<ExternalDrawing> list = drawingRepository.findByBatchTaskId(batchTaskId);
        if (!list.isEmpty()) {
