...
# 所有服务名称
all_service_names=(config auth docs notifications report stats bpm materialspm issues tasks)
...
if [[ ${argument_count} == 0 ]] || [[ "$1" != "start" ]] && [[ "$1" != "stop" ]]; then
    ...
    printf "服务名称：\n\t${c_bold}config${c_end}, ${c_bold}auth${c_end}, ${c_bold}docs${c_end}, ${c_bold}notifications${c_end}, ${c_bold}report${c_end}, ${c_bold}stats${c_end}, ${c_bold}bpm${c_end}, ${c_bold}materialspm${c_end}, ${c_bold}issues${c_end}, ${c_bold}tasks${c_end}\n\n"
...
for service_name in ${all_service_names[@]}; do
    ...
    if [[ "$1" == "start" ]]; then
        ...
        if [[ "${service_name}" == "ose-config" ]]; then
            memory_size="512m"
        elif [[ "${service_name}" == "ose-bpm" ]]; then
            memory_size="2048m"
        elif [[ "${service_name}" == "ose-stats" ]]; then
            memory_size="2048m"
        elif [[ "${service_name}" == "ose-tasks" ]]; then
            memory_size="2048m"
        fi

        # 启动服务
        ulimit -c unlimited
        java ${profile_name} -Xms${memory_size} -Xmx${memory_size} -verbose:gc -XX:+PrintGCDetails -jar ${service_jar_file} > ${service_log_file} &
